...
-   seems to be a dataset linking course grades with time spent (on an
    LMS?)
...
-   variables associated with that student
...
```{r}
d |> 
  select(student_id, percentage_earned) |> filter(percentage_earned > .1) |>
  arrange(desc(percentage_earned))
